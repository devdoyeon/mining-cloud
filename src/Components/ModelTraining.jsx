...
import { fileSetting, startFn, table2csv, previewThead, chart2png } from "js/common";
                ...
                <div className="content-wrap">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="fileUpload"
                            onChange={(e) => {
                                if (e.target.files.length === 1) return alert(`6개의 파일이 필요합니다.\n확인하신 뒤 다시 업로드해 주세요.`);
                                fileSetting(e, fileSettingState);
                            }}
                            accept=".csv"
                            multiple
                        />
                        <button onClick={(e) => training(e.target)} className={tab === "LR" ? "active" : "lr"}>
                            LR
                        </button>
                        <button onClick={(e) => training(e.target)} className={tab === "DT" ? "active" : "dt"}>
                            DT
                        </button>
                        <button onClick={(e) => training(e.target)} className={tab === "SVM" ? "active" : "svm"}>
                            SVM
                        </button>
                        <button onClick={(e) => training(e.target)} className={tab === "DNN" ? "active" : "dnn"}>
                            DNN
                        </button>
                        <button onClick={(e) => training(e.target)} className={tab === "RF" ? "active" : "rf"}>
                            RF
                        </button>
                        <button onClick={(e) => training(e.target)} className={tab === "XGBoost" ? "active" : "xg"}>
                            XGBoost
                        </button>
                        <br />
                        <p className="trainGuide">
                            <span className="bold">
                                AI 학습용 데이터셋 생성 <span className="bold highlight">Partitioning</span>
                            </span>
                            에서{" "}
                            <span
                        ...
                        <div className={`guideBox ${!guide.view ? "none" : ""}`}>
                            ...
                            y_val.csv, y_train.csv, y_test.csv, x_val.csv, x_train.csv, x_test.csv
                        </div>
                            ...
                            <div className="wrap">
                                <div className="train-chart chart">
                                    <ResponsiveHeatMap
                                        ...
                                        borderColor="black"
                                        labelTextColor={
                                            ({ data }) => (data.y > (matrix.max + matrix.min) / 2 ? "black" : "white")
                                ...
                                <div className="downloadBtnWrap">
                                    <button onClick={() => chart2png(fileInfo, tab)}>이미지 다운로드</button>
                                    <button onClick={() => table2csv(fileInfo, tab)}>표 다운로드</button>
                                </div>
                            </div>
                        )}
                    </div>
