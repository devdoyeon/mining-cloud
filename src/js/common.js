...
export const fileSetting = (e, { setFileInfo, setTab, setMsg, msg }) => {
    if (msg === "largeData") return alert("현재 보여지고 있는 데이터의 양이 많아\n파일을 변경할 수 없습니다.\n새로 고침 후 다시 시도해 주세요.");

    const file = e.target.files[0];
    if (!file) return;
    if (e.target.files[1]) {
        let nameArr = [];
        let trainName = ["y_val.csv", "y_train.csv", "y_test.csv", "x_val.csv", "x_train.csv", "x_test.csv"];
        let wrongNum = 0;
        ...
        if (nameArr.length !== 6) return alert(`6개의 파일이 필요합니다.\n확인하신 뒤 다시 업로드해 주세요.`);
        if (wrongNum > 0) return alert(`${wrongNum}개의 파일명이 잘못되었습니다.\n파일명을 확인해 주세요.`);
        setFileInfo({
            file: formData,
            name: nameArr,
        });
